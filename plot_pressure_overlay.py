# plot_scatter_fit_pressure.py
# Pressure (hPa): scatter + smoothed + solid linear fit (by flavor).
...
# ---- unified style ----
# Color palette (Okabe–Ito-ish subset) and visual constants used across plots
...
FRAME_LW  = 1.4  # plot frame (spines) line width

# ---- DB helpers ----
def safe_connect(db_path: str):
    """
    Open SQLite DB in read-only mode. If the DB is locked (e.g., logger running),
    make a 'snapshot' copy and open that, so plotting never blocks on locks.
    ...
    try:
        ...
        # fallback: copy the db file and read from the snapshot
...
def latest_exp_ids(con, n:int):
    """
    Return the latest N experiment IDs by the most recent measurement timestamp.
    """
...
def start_time(con, exp_id):
    """
    Get the alignment time (start of experiment).
    Prefer 'start_cook' event; if missing, fall back to the first measurement time.
    """
...
def series_press(con, exp_id)->pd.Series:
    """
    Load pressure time series (hPa) as a pandas Series indexed by timestamp.
    Drops NaNs and duplicate timestamps; sorted ascending.
    """
    df = pd.read_sql_query("SELECT ts_iso, press_hPa FROM measurements WHERE exp_id=? ORDER BY ts_iso",
                           con, params=(exp_id,))
    if df.empty: return pd.Series(dtype=float)
    s = pd.Series(df["press_hPa"].values, index=pd.to_datetime(df["ts_iso"]), name="press_hPa")
    s = s.dropna()
    s = s[~s.index.duplicated(keep="first")]
    return s.sort_index()

# ---- flavor parsing ----
# We look for a "flavor/base" string inside experiments.notes (JSON or KV text),
# or as a fallback inside a 'metadata'/'start_cook' event value.
KEYS = ["base","flavor","flavour"]

def _from_kv_text(txt):
    """
    Parse simple key-value text like 'flavor=tomato' or 'base: spicy'.
...
def _from_json_text(txt):
    """
    Try to parse a JSON dict and read keys in KEYS if present and non-empty.
    """
    try:
        obj = json.loads(txt)
...
def flavor_label(con, exp_id):
    """
    Resolve a human-readable flavor label for legend entries:
    1) experiments.notes (JSON or KV text)
    2) latest 'metadata' or 'start_cook' event value (JSON or KV text)
...
def smooth_series(s:pd.Series, w:int)->pd.Series:
    """
    Resample to 1 s, interpolate short gaps, then apply a centered rolling mean with window 'w' seconds.
    """
...
def linfit_xy(x,y):
    """
    Robust linear fit y = m*x + b with R^2.
    Falls back to lstsq if polyfit fails (e.g., odd data).
    Returns (m, b, r2) as floats (or NaN if not enough points).
    """
...
# ---- main ----
def main():
    ap = argparse.ArgumentParser(description="Pressure (hPa): scatter + smoothed + solid linear fit (by flavor).")
    # Core inputs and selection
    ap.add_argument("--db", default="project.db")      # SQLite file path
    ap.add_argument("--out", default=None)             # Output folder (default: alongside DB)
    ap.add_argument("--exp-ids", nargs="+")            # Explicit experiment IDs; if omitted, use latest N
    ap.add_argument("--n", type=int, default=4)        # How many latest experiments to include
    # Plot controls
    ap.add_argument("--downsample-sec", type=int, default=10)  # scatter points every N seconds
    ap.add_argument("--smooth-sec", type=int, default=30)      # moving-average window for trend
    ap.add_argument("--fit-minutes", type=float, default=20.0) # linear fit range from t=0 to this minute
    ap.add_argument("--x-max", type=float, default=40.0)       # crop axis to 0..x-max minutes
    # Side-placed slope/R^2 label block (axes fraction coordinates)
    ap.add_argument("--side-label-x", type=float, default=0.18)
    ap.add_argument("--side-label-y0", type=float, default=0.97)
    ap.add_argument("--side-label-dy", type=float, default=0.04)
    args = ap.parse_args()
    ...
    # Open DB (or snapshot if locked) and choose experiments
    ...
    slope_rows = []  # (flavor, color, formatted_text) to render as a stacked block

    for i,exp in enumerate(exp_ids):
        # Load pressure series
        s = series_press(con, exp)
        if s.empty: 
            continue
        ...
        # Align to start and crop to x_max minutes
        s = s[(s.index >= t0) & (s.index <= t0 + pd.to_timedelta(args.x_max, unit="m"))]
        if s.empty: 
            continue

        # Downsample for scatter (raw), and smooth for a trend line
        ...
        # Linear fit only on the first 'fit-minutes' window
    ...
    # Draw a boxed frame (all spines visible + black)
    for sp in ["top","right","left","bottom"]:
        ax.spines[sp].set_visible(True); 
        ax.spines[sp].set_linewidth(FRAME_LW); 
        ax.spines[sp].set_edgecolor("black")

    # Legend in the upper-left; slope/R^2 labels are stacked nearby to the right
    ...
    # Save figure
    plt.tight_layout()
    out_png = os.path.join(outdir, "overlay_scatter_fit_pressure.png")
